...
class TextTranslationTests(unittest.TestCase):
    ...
    def test_translates_disabled(self):
    ...
    def test_provider_backoff(self):
        with patch.object(text_translation_service, "ML_INTEREST_TRANSLATION_ENABLED", True), \
             patch.object(text_translation_service, "ML_INTEREST_TRANSLATION_PROVIDER", "libretranslate"), \
             patch.object(text_translation_service, "_provider_in_backoff", return_value=True):
            out = text_translation_service.translate_interest_text_for_ml("hello")
        self.assertEqual("hello", out.get("text"))
    ...
    def test_rate_limited(self):
        with patch.object(text_translation_service, "ML_INTEREST_TRANSLATION_ENABLED", True), \
             patch.object(text_translation_service, "ML_INTEREST_TRANSLATION_PROVIDER", "libretranslate"), \
             patch.object(text_translation_service._TRANSLATION_RATE_LIMITER, "check", return_value=(False, 0, 10.0)):
            out = text_translation_service.translate_interest_text_for_ml("hello")
        self.assertEqual("hello", out.get("text"))
